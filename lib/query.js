...
class Query {
  ...
  _compileObjectEl(el) {
    ...
    if (elKeys.indexOf('equals') > -1) {
      return this._compileEqualsFragment(el.equals);
    } else if (elKeys.indexOf('value') > -1) {
      return this._compileValueFragment(el.value);
    } else {
  ...
  _compileEqualsFragment(fragment) {
    return (function(args, paramIndexStart) {
      ...
      const skip = fragment.skip;

      let argKeys = Object.keys(args);
      if (Array.isArray(skip)) {
        argKeys = argKeys.filter(argKey => skip.indexOf(argKey) === -1);
      }
  ...
  _compileValueFragment(fragment) {
    return (function(args, paramIndexStart) {
      if (typeof paramIndexStart === 'undefined') {
        paramIndexStart = 1;
      }
      ...
      if (typeof value === 'undefined') {
        throw new Error('Query Execution Error: Missing value arg \'', key, '\'');
      }
